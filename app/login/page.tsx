...
export default function LoginPage() {
    ...
    async function handleSubmit(e: FormEvent) {
        ...
        try {
            ...
            setError(err instanceof FirebaseError ? getErrorMessage(err.code) : 'Sign in failed.');
        } finally {
            setLoading(false);
        }
    }
            ...
            <div style={{ width: '100%', maxWidth: '400px', display: 'flex', flexDirection: 'column', gap: '28px' }}>
                ...
                <div style={{ textAlign: 'center' }}>
                    ...
                    <p style={{ fontSize: '14px', color: 'var(--text-muted)', marginTop: '6px' }}>
                        Sign in to continue tracking your study sessions
                    </p>
                </div>
                ...
                <div style={{ borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-subtle)', backgroundColor: 'var(--bg-surface)', padding: '28px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <span style={{ fontSize: '12px', color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>or sign in with email</span>
                        <div style={{ flex: 1, height: '1px', backgroundColor: 'var(--border-subtle)' }} />
                    </div>
                    ...
                    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                            ...
                            <input type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} required autoComplete="email" style={inputStyle}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                            ...
                            <input type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required autoComplete="current-password" style={inputStyle}
                                onFocus={(e) => { e.currentTarget.style.borderColor = 'var(--accent-purple)'; }}
                                onBlur={(e) => { e.currentTarget.style.borderColor = 'var(--border)'; }} />
                        </div>
                ...
                <p style={{ textAlign: 'center', fontSize: '14px', color: 'var(--text-muted)' }}>
                    ...
                    <Link href="/register" style={{ color: 'var(--accent-purple)', textDecoration: 'none', fontWeight: 500 }}>Create one</Link>
                </p>
            </div>
        </main>
    );
